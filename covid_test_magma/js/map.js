...
function map(data, geoState, geoMSA, div, type) {
    ...
    const colorArray = Array.from(d3.schemeYlOrRd[breakPointArray.length]);
    const colorArrayTotal = Array.from(d3.schemeYlOrRd[breakPointArrayTotal.length]);
    colorArray.unshift('#8BC34A');
    colorArrayTotal.unshift('#8BC34A');

    const magmaColor_newcase = ['#fcfdbf',
        '#fecf92',
        '#fea16e',
        '#f7725c',
        '#e04c67',
    ...
    const colorscales = {'case':magmaColor_newcase,
        'death':magma_newdeath};


    const cScale = d3.scaleThreshold()
        .domain(breakPointArray)
        .range(colorscales[type]);
